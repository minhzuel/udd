...
export default function UserLayout({
  ...
  const navRoutes = useMemo<NavRoutes>(
    () => ({
      general: {
        title: 'Profile',
        icon: UserPen,
        path: `/apps/user/users/${id}`,
      },
      logs: {
        title: 'Activity Logs',
        icon: Activity,
        path: `/apps/user/users/${id}/logs`,
      },
    }),
    [id],
  );
  ...
  const { data: user, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/apps/user/users/${id}`);

      if (response.status == 404) {
        router.push('/apps/user/users');
      }
